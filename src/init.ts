...
interface ClientSpec {
  ...
  format: 'json' | 'toml';
  patch: (existing: unknown, entry: ProofsheetEntry) => unknown;
}

interface ProofsheetEntry {
  command: string;
  args: string[];
  env: Record<string, string>;
}

function jsonPath(...segments: string[]): string {
...
function expandConfigPath(candidates: string[]): string | null {
...
function readConfig(filePath: string, format: 'json' | 'toml'): unknown {
...
function writeConfig(filePath: string, data: unknown, format: 'json' | 'toml'): void {
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  const serialized =
    format === 'json'
      ? JSON.stringify(data, null, 2) + '\n'
      : tomlStringify(data as Record<string, unknown>);
  fs.writeFileSync(filePath, serialized);
}

function backupOnce(filePath: string): boolean {
  if (!fs.existsSync(filePath)) return false;
  const bak = `${filePath}.bak`;
  if (fs.existsSync(bak)) return false; // Don't overwrite existing backup.
  fs.copyFileSync(filePath, bak);
  return true;
}

function buildEntry(geminiKey: string, openaiKey: string): ProofsheetEntry {
...
function detectExistingKey(name: string): string {
  return process.env[name] ?? '';
}

async function gatherKeys(rl: readline.Interface): Promise<{ gemini: string; openai: string }> {
