...
function createMcpServer() {
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (req) => {
        ...
        const messages = (() => {
            if (name === 'art_direction')
                return [{ role: 'user', content: { type: 'text', text: ART_DIRECTION_PROMPT } }];
            if (name === 'refinement_picker')
                return [{ role: 'user', content: { type: 'text', text: REFINEMENT_PICKER_PROMPT } }];
            if (name === 'theme_builder')
                return [{ role: 'user', content: { type: 'text', text: THEME_BUILDER_PROMPT } }];
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (req) => {
        ...
        if (!fs.existsSync(themePath))
            throw new Error(`Theme not found: ${m[1]}`);
        return {
            contents: [
                {
                    uri,
...
async function handleRefineImage(args) {
    ...
    const sidecarPath = path.join(path.dirname(inputPath), '.meta', `${path.basename(inputPath)}.json`);
    let sidecar = null;
    ...
    const outputPath = args.output_path ??
        path.join(path.dirname(inputPath), `${path.basename(inputPath, path.extname(inputPath))}-${suffix}-${new Date()
            .toISOString()
            .replace(/[-:T]/g, '')
            .replace(/\..+/, '')}.png`);
...
function handleReadTheme(args) {
    ...
    if (theme.bestFor)
        parts.push(`Best for: ${theme.bestFor}`);
    parts.push('');
